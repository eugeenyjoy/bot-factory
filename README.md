...
По умолчанию сервер теперь принимает API-запросы только с локальной машины.

- Локальный запуск (без сети): `python app.py` (bind на `127.0.0.1:8000`)
- Разрешить удалённый доступ: `BF_ALLOW_REMOTE=1`
- Задать API-токен для всех `/api/*`: `BF_ADMIN_TOKEN=ваш_секрет`
  клиент должен передавать заголовок: `x-api-token: ваш_секрет`
- Системный файловый API (`/api/system/files`) по умолчанию выключен.
  Включить только при необходимости: `BF_ENABLE_SYSTEM_FILES_API=1`
- Кастомные host/port:
  `BF_HOST=127.0.0.1 BF_PORT=8000 python app.py`
