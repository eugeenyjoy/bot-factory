...
// ID бота который сейчас редактируем
let currentBotId = null;

// список моделей (загрузится с сервера)
let models = [];
...
document.addEventListener('DOMContentLoaded', () => {
    // загружаем модели и ботов при старте
    loadModels();
    loadBots();

    // обновляем список ботов каждые 10 секунд
    setInterval(loadBots, 10000);
});

// ====================================================
// API — обёртки запросов к серверу
// ====================================================
...
async function loadModels() {
    /**
     * Загружает список моделей и заполняет все <select>
     */
    try {
        models = await api('GET', '/api/models');
        fillModelSelects();
    } catch (e) {
        // если не загрузились — дефолтные
...
function fillModelSelects() {
    /**
     * Заполняет все выпадающие списки моделей
     */
    ...
    selects.forEach(selectId => {
        ...
        models.forEach(model => {
            ...
            // показываем имя + цену + метку цензуры
            const censored = model.censored ? ' 🔒' : ' 🔓';
            option.textContent = `${model.name} — ${model.price}${censored}`;
...
async function loadBots() {
    /**
     * Загружает список ботов и рисует карточки
     */
    try {
        const bots = await api('GET', '/api/bots');
        renderBots(bots);
    } catch (e) {
        // тихо — toast уже показан в api()
    }
}
...
function openCreateModal() {
    /**
     * Открывает модалку создания бота
     */
    // очищаем поля
...
async function createBot() {
    /**
    * Отправляет запрос на создание бота
    */
    ...
    // только имя и API ключ обязательны
...
async function openEditModal(botId) {
     ...
     * Загружает текущие настройки бота
     */
    currentBotId = botId;
    ...
    try {
        ...
        // загружаем данные для других табов
        loadUsersTab(botId);
        loadStatsTab(botId);
                ...
                <tbody>
                    ${users.map(u => `
                        <tr>
                            ...
                            <td>
                                <label class="toggle">
                                    <input type="checkbox" ${u.is_vip ? 'checked' : ''} onchange="toggleVip('${botId}', ${u.user_id}, this.checked)">
                                    <span class="toggle-slider"></span>
                                </label>
                            </td>
                            <td style="font-size:12px; color:#8b8b8b;">${u.last_seen || '—'}</td>
                            <td>
                                <button class="btn btn-danger btn-sm" onclick="clearUser('${botId}', ${u.user_id})">🗑️</button>
                            </td>
                        </tr>
                    `).join('')}
                </tbody>
...
async function toggleVip(botId, userId, isVip) {
    /**
     * Переключает VIP статус юзера
     */
    try {
        await api('POST', `/api/bots/${botId}/vip`, {
            user_id: userId,
            is_vip: isVip
        });
...
async function clearUser(botId, userId) {
    /**
     * Сбрасывает данные юзера
     */
    ...
    try {
        ...
        container.innerHTML = `
            <div class="stat-card">
                ...
                <div class="stat-label">Юзеров</div>
            </div>
            <div class="stat-card">
            ...
            <div class="stat-card">
                <div class="stat-value">${stats.paying_users}</div>
                <div class="stat-label">Платящих</div>
            </div>
            <div class="stat-card">
                <div class="stat-value">${stats.total_revenue_stars} ⭐</div>
...
function switchTab(tabName) {
    /**
     * Переключает табы в модалке редактирования
     */
    // убираем active у всех табов
    document.querySelectorAll('.tab').forEach(t => t.classList.remove('active'));
    document.querySelectorAll('.tab-content').forEach(t => t.classList.remove('active'));

    // ставим active нужному
    document.getElementById(`tab-${tabName}`).classList.add('active');

    // находим кнопку таба и делаем active
...
function toast(message, type = 'info') {
    /**
     * Показывает уведомление внизу справа
...
function escapeHtml(text) {
    /**
     * Экранирует HTML чтобы избежать XSS
     */
...
async function loadKnowledge() {
    ...
    try {
        ...
        document.getElementById('knowledgeList').innerHTML =
            '<p style="color:#f85149;">Бот не активен. Сначала запустите.</p>';
    }
}
    ...
    list.innerHTML = `
        ...
        ${data.files.map(f => `
            <div style="display:flex; justify-content:space-between; align-items:center;
                        padding:8px 12px; background:#1a1a1a; border-radius:8px; margin-bottom:4px;">
                <div>
                    <span style="color:#e0e0e0; font-size:13px;">📄 ${escapeHtml(f.name)}</span>
                    <span style="color:#555; font-size:11px; margin-left:8px;">
                        ${formatFileSize(f.size)} · ${f.chunks} чанков
                    </span>
                </div>
                <button class="btn btn-danger btn-sm"
                        onclick="deleteKnowledgeFile('${escapeHtml(f.name)}')">✕</button>
            </div>
        `).join('')}
    `;
}
...
// загрузка файла через drag & drop или кнопку
...
async function handleFileUpload(event) {
    ...
    for (const file of files) {
        ...
        try {
            ...
            if (result.ok) {
                toast(`✅ ${file.name}: ${result.chunks} чанков`, 'success');
            } else {
...
async function addKnowledgeText() {
    ...
    try {
        const result = await api('POST', `/api/bots/${knowledgeBotId}/knowledge/text`, {
            name: name,
            text: text
        });
...
async function testSearch() {
    ...
    try {
        const data = await api('POST', `/api/bots/${knowledgeBotId}/knowledge/search`, {
            query: query
        });
